...
Require Import op_defs dotprod_models float_accuracy_lemmas list_lemmas real_lemmas.
...
Definition g (t: type) (n: nat) (r : R) : R := error_rel t (n + 1) r.
...
Lemma fma_dotprod_error': 
  ...
  Rabs (FT2R fp - rp) <=  g t (length v1) rp_abs.
Proof.
intros t v1 v2 Hlen.
...
unfold g, error_rel; rewrite !Zle_imp_le_bool.
set (D:= default_rel t).
set (E:= default_abs t).
...
Lemma fma_dotprod_error: 
  ...
  (forall xy, In xy (List.combine v1 v2) ->   
      Binary.is_finite _ _ (fst xy) = true /\ Binary.is_finite _ _ (snd xy) = true) ->
  (forall x y z, BFMA x y z = (fma_dotprod t v1 v2) -> Binary.is_finite _ _ (fma_dotprod t v1 v2) = true /\
      Rabs (rounded t (FT2R x * FT2R y + FT2R z)) < ov) ->      
  Binary.is_finite (fprec t) (femax t) (fma_dotprod t v1 v2) = true /\
    Rabs (FT2R (fma_dotprod t v1 v2) - sum prods) <= g t (length v1) (sum abs_prods).
Proof.
...
Lemma fma_dotprod_mixed_error: 
  ...
  Binary.is_finite (fprec t) (femax t) fp = true /\
  exists delta eta, 
    Rabs delta <= g t (length v1) rp_abs /\ 
    Rabs eta <= g1 t (length v1) rp_abs /\
    FT2R fp = rp * (1 + delta) + eta.
Proof.
